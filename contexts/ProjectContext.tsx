...
interface ProjectContextType {
  ...
  addProgressEntry: (
    projetoID: number,
    formData: FormData
  ) => Promise<{ success: boolean; porcentagem?: number; error?: string }>;
  removeProgressImage: (
    projetoID: number,
    imagemID: number
...
export const ProjectProvider: React.FC<ProjectProviderProps> = ({
  ...
  const deleteProject = async (projetoID: number): Promise<boolean> => {
    ...
    try {
      ...
      if (result.success) {
        setProjects((prev) =>
          prev.filter((project) => project.projetoID !== projetoID)
        );
        return true;
      } else {
  ...
  const removeProgressImage = async (
    projetoID: number,
    imagemID: number
  ): Promise<{ success: boolean; error?: string }> => {
    setIsLoading(true);
    ...
    try {
      ...
      if (result.success) {
        ...
        if (updated) {
          setProjects((prev) =>
            prev.map((p) => (p.projetoID === projetoID ? updated : p))
          );
        }
        return { success: true };
      } else {
  ...
  const refreshProjects = (nomeGrupo?: string) => {
    ...
    return apiService
      .getProjects(nomeGrupo)
      .then((result) => {
        if (result.success && result.data) {
          // normalize projects list
          const normalizedList = (result.data.projetos || []).map((p: any) =>
            normalizeProject(p)
          );
  ...
  function normalizeProject(p: any) {
    ...
    const max_progress =
      imagensProgresso.length > 0
        ? Math.max(...imagensProgresso.map((i: any) => i.porcentagem || 0))
        : 0;
